  ...
  useEffect(() => {
    const fetchAdverts = async () => {
      try {
        ...
        const vendorAdverts = await apiGetAds()
  ...
  return (
    ...
    {/* <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-md rounded-md">  */}
      ...
      <div onSubmit={handleSubmit} key={index} className="bg-white bg-opacity-20 backdrop-blur-md rounded-lg shadow-2xl p-8 space-y-4 glass">
             <img className="p-2 border-2 w-[35] h-[full]"
              src={`https://savefiles.org/${advert.image}?shareable_link=449`}
              alt={advert.title}
           
            />
            <h2 className="text-1xl ">Product: {advert.title}</h2>
            <p className="text-1xl ">Category: {advert.category}</p>
